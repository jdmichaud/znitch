...
pub const BpfLoader = struct {
    ...
    pub fn lookupDnsName(self: *BpfLoader, pid: u32) ?[]const u8 {
        ...
        // Only use per-PID match — the process that did the DNS query must be
        // the same process that's connecting. The global fallback was wrong:
        // it would label unrelated connections with the most recent DNS query
        // from any process.
