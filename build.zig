...
pub fn build(b: *std.Build) void {
    ...
    bpf_prog.addCSourceFile(.{
        ...
        .flags = &.{
            ...
            "-O2",
            "-fno-asynchronous-unwind-tables", // prevent .eh_frame in BPF object
        },
    });

    // BPF program needs libbpf's BPF-side headers
    const libbpf_artifact = libbpf_dep.artifact("bpf");
    portableAddIncludePath(bpf_prog, libbpf_artifact.getEmittedIncludeTree());
    ...
    // Test step — unit tests for DNS parsing, event structs, D-Bus marshaling, etc.
...
fn portableLinkSystemLibrary(c: *std.Build.Step.Compile, name: []const u8) void {
